...
double reorder_self_energy(string file_name, const int number_orbitals){
    ...
    //qstd::cout << (double)(count - 1) / ((number_orbitals + 1) * 2.0) << std::endl;
...
void get_sigma(Parameters &parameters, std::vector<std::vector<dcomp>> &sigma_mb_r_up, std::vector<std::vector<dcomp>> &sigma_mb_r_down,
    ...
    ifstream file("textfiles/SigmaMBlesser.dat");
    bool sigma_lesser;
    ...
    parameters.steps = reorder_self_energy("textfiles/SigmaMB", parameters.num_orbitals);

    if (sigma_lesser == true) {
        parameters.steps = reorder_self_energy("textfiles/SigmaMBlesser", parameters.num_orbitals);
    }
    ...
    read_self_energy(parameters, sigma_mb_r_up, sigma_mb_r_down, "textfiles/SigmaMB");
    read_self_energy(parameters, sigma_mb_l_up, sigma_mb_l_down, "textfiles/SigmaMBlesser");    
}
